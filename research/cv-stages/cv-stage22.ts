// cv-stage22: stage 11's working pattern wrapped in IIFE. Single-variable test:
// does putting the script-tag setup inside a function break it?

const session = 'cv22' + Math.random().toString(36).slice(2, 8);
...
line('tick', `Stage22: Promise return + .then`);

// THE TEST: stage 18 IIFE pattern BUT signal completion via a Promise resolver
// + .then() handler. Match stage 19's Promise return / .then-handler structure
// but keep stage 18's synchronous onRuntimeInitialized + onload behavior.
type CvMod = { Mat: new () => { rows: number; delete: () => void } };
const cvPromise = new Promise<CvMod>((resolve, reject) => {
  ...
  s.onerror = (e) => { line('err', `<script> onerror: ${e}`); reject(new Error('script onerror')); };
  ...
  const poll = setInterval(() => {
    ...
    if (cv && cv.Mat) {
      clearInterval(poll);
      resolve(cv as CvMod);  // ← Promise resolver instead of synchronous signal
    } else {
      line('tick', `poll: cv=${typeof cv}, keys=${keys}, Mat=${typeof cv?.Mat}`);
    }
  }, 2000);

  setTimeout(() => { reject(new Error('STAGE22 TIMEOUT')); }, 30_000);
});

cvPromise.then((cv) => {
  const m = new cv.Mat();
  line('ok', `STAGE22 PASSED: cv.Mat at +X. rows=${m.rows}`);
  m.delete();
  verdict.className = 'ok';
  verdict.textContent = `✓ STAGE22 PASSED`;
}).catch((e) => {
  line('err', `STAGE22 FAIL: ${e.message}`);
  verdict.className = 'err'; verdict.textContent = `✗ STAGE22 FAIL`;
});
