// cv-stage18: stage 11's working pattern wrapped in IIFE. Single-variable test:
// does putting the script-tag setup inside a function break it?

const session = 'cv18' + Math.random().toString(36).slice(2, 8);
...
line('tick', `Stage18: stage 11 inside IIFE`);
...
(function setupCv() {
  ...
  const poll = setInterval(() => {
    ...
    if (cv && cv.Mat) {
      ...
      try {
        const m = new cv.Mat();
        line('ok', `STAGE18 PASSED: cv.Mat at +${((performance.now() - t1) / 1000).toFixed(2)}s. rows=${m.rows}, keys=${keys}`);
        m.delete();
        verdict.className = 'ok';
        verdict.textContent = `✓ STAGE18 PASSED`;
      } catch (e) {
        line('err', `STAGE18 FAIL: ${(e as Error).message}`);
  ...
  setTimeout(() => {
    if (verdict.className !== 'ok') {
      clearInterval(poll);
      verdict.className = 'err'; verdict.textContent = `✗ STAGE18 TIMEOUT`;
      line('err', 'STAGE18 TIMEOUT 30s');
    }
  }, 30_000);
})();
