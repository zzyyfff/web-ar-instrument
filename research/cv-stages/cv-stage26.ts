// cv-stage26: stage 25 but use cvLib.startCvPreload() instead of inline cv-load.
...
const sessionId = 'cv26' + Math.random().toString(36).slice(2, 8);
...
line('tick', `Stage26: stage 25 + USE cvLib.startCvPreload() (no inline)`);

// Calibrator-style diag pipeline (same as stage 13/14)
const diagSession = 'calib26_' + Math.random().toString(36).slice(2, 8);
console.log(`[stage26] diag session: ${diagSession}`);
...
cvLib.startCvPreload().then((cv) => {
  ...
  line('ok', `STAGE26 PASSED: cv.Mat at +${((performance.now() - t1) / 1000).toFixed(2)}s. rows=${m.rows}`);
  m.delete();
  verdict.className = 'ok';
  verdict.textContent = `✓ STAGE26 PASSED`;
}).catch((e) => {
  line('err', `STAGE26 FAIL: ${(e as Error).message}`);
  verdict.className = 'err';
  verdict.textContent = `✗ STAGE26 FAIL`;
});

setTimeout(() => {
  if (verdict.className !== 'ok') {
    line('err', `STAGE26 TIMEOUT at 30s`);
    verdict.className = 'err';
    verdict.textContent = `✗ STAGE26 TIMEOUT`;
  }
}, 30_000);
