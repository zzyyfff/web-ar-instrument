// cv-stage20: stage 11's working pattern wrapped in IIFE. Single-variable test:
// does putting the script-tag setup inside a function break it?

const session = 'cv20' + Math.random().toString(36).slice(2, 8);
...
line('tick', `Stage20: stage 18 + idle module-level lets`);

// THE ADDITION: declare module-level lets but don't use them.
let _setupDone = false;
let _resolvedCv: unknown = null;
let _rejectedErr: unknown = null;
const _readyCallbacks: unknown[] = [];
void _setupDone; void _resolvedCv; void _rejectedErr; void _readyCallbacks;

// THE TEST: same code as stage 11 but wrapped in an immediately-invoked function.
(function setupCv() {
  ...
  const poll = setInterval(() => {
    ...
    if (cv && cv.Mat) {
      ...
      try {
        const m = new cv.Mat();
        line('ok', `STAGE20 PASSED: cv.Mat at +${((performance.now() - t1) / 1000).toFixed(2)}s. rows=${m.rows}, keys=${keys}`);
        m.delete();
        verdict.className = 'ok';
        verdict.textContent = `✓ STAGE20 PASSED`;
      } catch (e) {
        line('err', `STAGE20 FAIL: ${(e as Error).message}`);
  ...
  setTimeout(() => {
    if (verdict.className !== 'ok') {
      clearInterval(poll);
      verdict.className = 'err'; verdict.textContent = `✗ STAGE20 TIMEOUT`;
      line('err', 'STAGE20 TIMEOUT 30s');
    }
  }, 30_000);
})();
