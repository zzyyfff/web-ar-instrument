// cv-stage25: stage 14 but use cvLib.startCvPreload() instead of inline cv-load.
...
const sessionId = 'cv25' + Math.random().toString(36).slice(2, 8);
...
line('tick', `Stage25: stage 14 + USE cvLib.startCvPreload() (no inline)`);

// Calibrator-style diag pipeline (same as stage 13/14)
const diagSession = 'calib25_' + Math.random().toString(36).slice(2, 8);
console.log(`[stage25] diag session: ${diagSession}`);
...
cvLib.startCvPreload().then((cv) => {
  ...
  line('ok', `STAGE25 PASSED: cv.Mat at +${((performance.now() - t1) / 1000).toFixed(2)}s. rows=${m.rows}`);
  m.delete();
  verdict.className = 'ok';
  verdict.textContent = `✓ STAGE25 PASSED`;
}).catch((e) => {
  line('err', `STAGE25 FAIL: ${(e as Error).message}`);
  verdict.className = 'err';
  verdict.textContent = `✗ STAGE25 FAIL`;
});

setTimeout(() => {
  if (verdict.className !== 'ok') {
    line('err', `STAGE25 TIMEOUT at 30s`);
    verdict.className = 'err';
    verdict.textContent = `✗ STAGE25 TIMEOUT`;
  }
}, 30_000);
