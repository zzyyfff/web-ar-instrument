// cv-stage24: stage 11's working pattern wrapped in IIFE. Single-variable test:
// does putting the script-tag setup inside a function break it?

const session = 'cv24' + Math.random().toString(36).slice(2, 8);
...
line('tick', `Stage24: IIFE setup + extracted Promise resolver`);

// Extract Promise resolver to outer scope. Script setup happens in IIFE.
// IIFE references the OUTER resolver, not a Promise-constructor-scope one.
type CvMod = { Mat: new () => { rows: number; delete: () => void } };
let extResolve: ((cv: CvMod) => void) | null = null;
let extReject: ((e: Error) => void) | null = null;
const cvPromise = new Promise<CvMod>((res, rej) => { extResolve = res; extReject = rej; });
cvPromise.then((cv) => {
  const m = new cv.Mat();
  line('ok', `Stage24 .then fired: rows=${m.rows}`);
  m.delete();
});
...
(function setupCv() {
  ...
  s.onerror = (e) => { line('err', `<script> onerror: ${e}`); extReject?.(new Error('onerror')); };
  ...
  const poll = setInterval(() => {
    ...
    if (cv && cv.Mat) {
      clearInterval(poll);
      try {
        line('ok', `STAGE24 PASSED: cv.Mat at +${((performance.now() - t1) / 1000).toFixed(2)}s. keys=${keys}`);
        verdict.className = 'ok';
        verdict.textContent = `✓ STAGE24 PASSED`;
        extResolve?.(cv as CvMod);  // ← resolves the OUTER-scope Promise
      } catch (e) {
        line('err', `STAGE24 FAIL: ${(e as Error).message}`);
  ...
  setTimeout(() => {
    if (verdict.className !== 'ok') {
      clearInterval(poll);
      verdict.className = 'err'; verdict.textContent = `✗ STAGE24 TIMEOUT`;
      line('err', 'STAGE24 TIMEOUT 30s');
    }
  }, 30_000);
})();
