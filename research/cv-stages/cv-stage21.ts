// cv-stage21: stage 11's working pattern wrapped in IIFE. Single-variable test:
// does putting the script-tag setup inside a function break it?

const session = 'cv21' + Math.random().toString(36).slice(2, 8);
...
line('tick', `Stage21: empty onRuntimeInitialized`);
...
(function setupCv() {
  ...
  // THE TEST: empty onRuntimeInitialized (matches stage 19's behavior when
  // cv.Mat isn't yet set — does nothing).
  w.Module = {
  ...
  const poll = setInterval(() => {
    ...
    if (cv && cv.Mat) {
      ...
      try {
        const m = new cv.Mat();
        line('ok', `STAGE21 PASSED: cv.Mat at +${((performance.now() - t1) / 1000).toFixed(2)}s. rows=${m.rows}, keys=${keys}`);
        m.delete();
        verdict.className = 'ok';
        verdict.textContent = `✓ STAGE21 PASSED`;
      } catch (e) {
        line('err', `STAGE21 FAIL: ${(e as Error).message}`);
  ...
  setTimeout(() => {
    if (verdict.className !== 'ok') {
      clearInterval(poll);
      verdict.className = 'err'; verdict.textContent = `✗ STAGE21 TIMEOUT`;
      line('err', 'STAGE21 TIMEOUT 30s');
    }
  }, 30_000);
})();
