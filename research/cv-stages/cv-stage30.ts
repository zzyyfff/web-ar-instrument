// cv-stage30: inline cv-load IIFE that ONLY writes to a module-level let.
...
const session = 'cv30' + Math.random().toString(36).slice(2, 8);
...
line('tick', `Stage30: cv-load writes to module let only; async caller polls`);
...
async function pollUntilCvReady(): Promise<CvMod> {
  ...
  while (!cvReady && !cvErr) {
    pollN++;
    if (pollN % 5 === 0) line('tick', `caller-poll n=${pollN} cvReady=${!!cvReady}`);
    await new Promise<void>((r) => setTimeout(r, 100));
  }
  line('tick', `caller-poll EXITED at n=${pollN} cvReady=${!!cvReady} cvErr=${!!cvErr}`);
...
(async () => {
  ...
  try {
    ...
    const m = new cv.Mat();
    line('ok', `STAGE30 PASSED: cv.Mat via poll at +${((performance.now() - t2) / 1000).toFixed(2)}s. rows=${m.rows}`);
    m.delete();
    verdict.className = 'ok';
    verdict.textContent = `✓ STAGE30 PASSED`;
  } catch (e) {
    line('err', `STAGE30 FAIL: ${(e as Error).message}`);
...
setTimeout(() => {
  if (verdict.className !== 'ok') {
    verdict.className = 'err'; verdict.textContent = `✗ STAGE30 TIMEOUT`;
    line('err', 'STAGE30 outer timeout 35s');
  }
}, 35_000);
